...
charts <- read_csv("C:/Users/luker/OneDrive - Tulane University/Desktop/Business Analytics Practicum/Music Project/charts.csv")

#Selecting only the rows in the top200 category
top200 <- charts %>% filter(chart == "top200")

#check for null values
data.frame(missing = sapply(top200, function(x) sum(length(which(is.na(x))))))


#We find that the only null values are from a Spotify song literally called "NA". This is an error on R's side
top200 %>% filter(is.na(title))

#Thus, we make sure that ONLY blank values are counted as null values
...
#We can now continue our analysis, filtering data by year to make RStudio load variables faster
year_top200 <- top200 %>% mutate(year = format(date, "%Y"))

Seventeen_data <- year_top200 %>%  filter(year == 2017)
#write.csv(Seventeen_data, "C:/Users/luker/OneDrive/TUL FA 24/Business Analytics Practicum/Seventeen1.csv", row.names = T)
Eighteen_data <- year_top200 %>% filter(year == 2018)
#write.csv(Eighteen_data, "C:/Users/luker/OneDrive/TUL FA 24/Business Analytics Practicum/Eighteen.csv", row.names = T)
Nineteen_data <- year_top200 %>% filter(year== 2019)
#write.csv(Nineteen_data, "C:/Users/luker/OneDrive/TUL FA 24/Business Analytics Practicum/Nineteen.csv", row.names = T)
...
#Optional code (Only used this for our decision to choose BTS): Finding the artists on the top200 on the most countries, then filtering by 25th-30th positions
#to find an example of an artists who are consistently popular with room for growth 
#We found that BTS is a popular group that still has room for growth. 
